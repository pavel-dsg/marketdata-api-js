...
module.exports = (() => {
	...
	function ConnectionInternal(marketState) {
		...
		function connect(server, username, password) {
			...
			__connection.onopen = () => {
				...
				// TODO Hack
				var loginReq = {
					"loginRequest": { "username": "***openfeed1", "password": "password" }
				};
			...
			__connection.onmessage = (event) => {
				...
				if (event.data instanceof ArrayBuffer) {
					...
					__logger.info("< " + JSON.stringify(message));
				...
				if (message) {
					if (__config && __config.isOpenfeed()) {
						processOpenfeedMessage(message);
					}
					else {
						// DDF
		 ...
		 * 
		*/
		function processOpenfeedMessage(ofMessage) {
	...
	class Connection extends ConnectionBase {
		...
		_connect(webSocketAdapterFactory) {
			...
			if (this._config && this._config.protocol === "openfeed") {
				// Openfeed Codec
