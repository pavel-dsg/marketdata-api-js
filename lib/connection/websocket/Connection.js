...
const OpenfeedCodec = require('./adapter/OpenfeedCodec');
	...
	function ConnectionInternal(marketState) {
		...
		*/
		...
		function openfeedHeart(rsp) {
			let message = __openfeedConverter.convertHeartBeat(ofMessage);
			parseMarketMessage(message);
		}
		function openfeedInstrumentDefinition(ofMessage) {
			__logger.info("INST < " + JSON.stringify(ofMessage,null,4));
		...
		function openfeedMarketSnapshot(ddfSymbol, definition, ofMessage) {
			__logger.info("SNAP < " + JSON.stringify(ofMessage,null,4));
			let messages = __openfeedConverter.convert(ddfSymbol,definition, ofMessage);
			messages.forEach ( (m) => {
				parseMarketMessage(m);
			});
			// TODO Convert Depth from Snapshot
			// if(ofMessage.marketSnapshot.priceLevels && ofMessage.marketSnapshot.priceLevels.length > 0) {
			// 	message = __openfeedConverter.convertSnapshotDepth(ddfSymbol,definition, ofMessage);
			// 	parseMarketMessage(message);
			// }
			// // Cumulative Volume, how to check for?
			// message = __openfeedConverter.convertSnapshotCumulativeVolume(ddfSymbol,definition, ofMessage);
			// TODO
	...
	class Connection extends ConnectionBase {
		...
		_connect(webSocketAdapterFactory) {
			...
			if (factory === null) {
				...
				factory = new WebSocketAdapterFactoryForBrowsers();
			}
			if (this._config && this._config.isOpenfeed()) {
				// Openfeed Codec
				let codec = new OpenfeedCodec();
