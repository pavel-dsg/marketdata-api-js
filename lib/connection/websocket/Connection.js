	...
	function ConnectionInternal(marketState) {
		...
		*/
		function openfeedProcessResponseMessage(ofMessage) {
			...
			switch (ofMessage.data) {
				...
				case "marketSnapshot":
					...
					openfeedMarketSnapshot(definition,ofMessage);
					break;
				case "marketUpdate":
					definition = getDefinition(ofMessage.marketUpdate.marketId);
					openfeedMarketUpdate(definition,ofMessage);
					break;
				default:
			}
		}
		...
		function openfeedInstrumentDefinition(ofMessage) {
			...
			__symbolToDefinition[def.symbol] = def;
		}
		function openfeedMarketSnapshot(definition,ofMessage) {
			let message = __openfeedConverter.convert(definition,ofMessage);
			parseMarketMessage(message);
		}
		function openfeedMarketUpdate(definition,ofMessage) {
			let message = __openfeedConverter.convert(definition,ofMessage);
			parseMarketMessage(message);
		}
